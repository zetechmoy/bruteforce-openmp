...
// gcc shadowFaker.c -o shadowFaker -Wall -lcrypto
// ./shadowFaker shadow.txt fakeShadow.sha 1000
...
int main(int argc, char *argv[]) {
	...
	while (number_of_shadows < wanted_shadows){
		...
		while (currline!=NULL && (number_of_shadows < wanted_shadows)){
			if (rand() % CHANCE ==0){
				if (!(number_of_shadows % ratio)){ // just a little salt to add
				...
				SHA1((const unsigned char *) currline, length, hash);
